...
class RsTheFluidDynamicsSliderFront extends RevSliderFunctions {
	...
	private function isEnabled($slider){
		...
		foreach($slides as $slide){
			...
			foreach($layers as $layer){
				if($this->get_val($layer, 'subtype', false) === 'fluiddynamics'){
					$enabled = true;
					break;
				}
			}
	...
	public function add_addon_files($html, $export){
		...
		$_jsPathMin = file_exists(RS_FLUIDDYNAMICS_PLUGIN_PATH . 'public/assets/js/revolution.addon.' . $this->pluginTitle . '.js') ? '' : '.min';
		if(!$export->usepcl){
			$export->zip->addFile(RS_FLUIDDYNAMICS_PLUGIN_PATH . 'public/assets/js/revolution.addon.' . $this->pluginTitle . $_jsPathMin . '.js', 'js/revolution.addon.' . $this->pluginTitle . $_jsPathMin . '.js');
		}else{
			$export->pclzip->add(RS_FLUIDDYNAMICS_PLUGIN_PATH.'public/assets/js/revolution.addon.' . $this->pluginTitle . $_jsPathMin . '.js', PCLZIP_OPT_REMOVE_PATH, RS_FLUIDDYNAMICS_PLUGIN_PATH.'public/assets/js/', PCLZIP_OPT_ADD_PATH, 'js/');
		}
		
		$html = str_replace(array(RS_FLUIDDYNAMICS_PLUGIN_PATH.'public/assets/js/revolution.addon.' . $this->pluginTitle . '.min.js', RS_FLUIDDYNAMICS_PLUGIN_PATH.'public/assets/js/revolution.addon.' . $this->pluginTitle . '.js'), $export->path_js .'revolution.addon.' . $this->pluginTitle . $_jsPathMin . '.js', $html);
	...
	public function add_html_script_additions($return, $output){
		...
		$return['toload']['fluiddynamics'] = '<script'. $addition .' src="'. $this->pluginUrl . 'public/assets/js/revolution.addon.' . $this->pluginTitle . $_jsPathMin . '.js"></script>';
		$return['toload']['threejs'] = '<script'. $addition .' src="'. RS_PLUGIN_URL . 'public/assets/js/libs/three.min.js"></script>';
		$return['toload']['fluiddynamicsgpu'] = '<script'. $addition .' src="'. $this->pluginUrl . 'public/assets/js/GPUComputationRenderer.js"></script>';
	...
	public function write_footer_scripts(){
		echo '<script type="text/javascript">window.RVS = window.RVS || {}; window.RVS.ENV = window.RVS.ENV || {}; window.RVS.ENV.FLUIDDYNAMICS_URL = "'.RS_FLUIDDYNAMICS_PLUGIN_URL.'";</script>'."\n";
	}
	...
	public function write_init_script($slider, $id){
		...
		if(!empty($addOn)){
			...
			echo "\n";
			echo "\t\t\t\t\t\t" . 'if (revapi'.$id.' !== undefined) jQuery.fn.revolution.fluidDynamicsInit(revapi'.$id.'[0].id,  {url:"'.RS_FLUIDDYNAMICS_PLUGIN_URL.'"});' . "\n";
